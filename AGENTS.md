...
- **사용 통계를 단독 증거로 쓰지 마세요.** 통계는 리셋 시점 이후만 유효합니다.
  `since`를 함께 실어 소비자가 스스로 판단하게 합니다. usage가 없는 것과
  관측된 0은 다른 사실입니다 — 없으면 키를 빼고, 0이면 usage를 싣습니다.
- **`limitations`를 장식으로 쓰지 마세요.** 파싱 실패 routine 수, 통계 부재,
  드라이버가 숨긴 카탈로그 — 그 DB에서 **실제로 세어서** 만듭니다. 매번 붙는
  상투적 경보는 읽히지 않습니다. 통계가 꺼진 환경(`track_functions=none`,
  `performance_schema=OFF`)도 limitation 대상입니다 — 0행을 "관측된 0"으로
  오독하게 두지 마세요.
- **유령 정점 id를 만들지 마세요.** 정점 id는 reader가 부여한 것만 존재합니다.
  이름 충돌로 `name@kind`로 분리된 정점을 파서가 옛 id로 찾으면 간선이
  허공을 가리킵니다 — `resolve_renamed`처럼 실제 정점을 확인하고, 없으면
  간선 대신 notes로 남기세요.
