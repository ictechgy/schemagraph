...
- 2026-09-22: **후속 정확도·규모 검증 진행 중** (`feature/sqlserver-oracle-scale-validation`).
  ...
  - 정확도·공통 문서·CI는 메인 에이전트가, `Scripts/benchmark-scale.py`·
    `Scripts/test-benchmark-scale.py`·`PERFORMANCE.md`는 별도 작업자가 맡는다.
    최종 성능 측정은 DB 기동·빌드와 겹치지 않는 시간에 실행한다.
...
이후 사용자가 아래 두 후속 과제의 실행을 승인했다.

1. SQL Server·Oracle 공개 표본과 저장 프로시저·trigger 정확도 평가를 확장한다.
   실제 SQL 유효성·독립 기대값을 먼저 고정하고, 첫 결과·수정 결과·실제 한계를
   분리해 기록한다. 로컬 플랫폼에서 실행할 수 없는 DB는 CI의 실제 서버로 검증한다.
2. 단일 대형 스키마·밀집 그래프에서 메모리·반복 조회·취소 응답 시간을 측정한다.
   입력·바이너리 식별자, 독립 출력 검증, 반복값과 조건을 남기며 측정을 성능 개선으로
   오인하지 않는다. 임의의 하드웨어 성능 수치를 CI 통과 기준으로 만들지 않는다.

현재 완료된 CI 수정은 런타임을 바꾸지 않아 새 릴리스가 필수가 아니다.
후속 평가에서 런타임 수정이 생기면 배포 필요성을 별도로 기록한다.
