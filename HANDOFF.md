...
- 2026-09-22: **PR #14 리뷰 후 머지 승인**.
  ...
  확장했다. 양쪽 값 원천의 회귀와 별도 SQL Server 실DB 문법/행 검증을 추가했다.
  고정된 16개 기대값은 바꾸지 않았다. main 머지가 변경된 프로브를 기존 0.4.3으로
  다시 게시하려는 Pages 작업도 수동 릴리스 태그 빌드로 한정했다. 원래의
  immutable 바이트 검사는 유지하며, 머지 자체는 새 배포를 요청하지 않는다.
  최종 코드·검증·병합 상태는 [PR #14](https://github.com/ictechgy/schemagraph/pull/14)를
  기준으로 확인한다. 변경 없는 입력의 이전 검증은 재사용한다.
