...
[정확도 평가](ACCURACY.md)는 공개 PostgreSQL·SQLite·MySQL·MariaDB 스키마를
독립적으로 검토한 SQL 기대값과 DB가 제공하는 참조 기록에 대조합니다.
...
| `lint` | FK 인덱스 prefix와 선언 타입, PK 없는 테이블, 중복 인덱스 후보, 미해결·모호한 참조 사실을 검사합니다. |
| `merge <documents>…` | source ID별로 구분한 DB 카탈로그를 통합합니다. |
| `openlineage --namespace <uri>` | 컬럼 계보를 데이터 카탈로그용 OpenLineage RunEvent(NDJSON)로 내보냅니다. |
| `facts --document <path>` | 카탈로그 선언을 isthmus persistence bridge-facts 문서(`relation-decl`, platform `sql`)로 보냅니다. 각 `symbol.usr`는 그래프 정점 id입니다. |
...
0.4.3 이상에서는 `--url-env SG_DATABASE_URL`로 export된 환경 변수에서
