...
Use the v0.6.0 Action or pin a reviewed full commit containing `action.yml`;
v0.4.3 does not contain it.
For an immutable source pin, replace `v0.6.0` below with its verified full commit SHA.
...
jobs:
  schema-review:
    ...
    steps:
      ...
      - id: schema
        uses: ictechgy/schemagraph@v0.6.0
        with:
